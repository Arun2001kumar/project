...
class FaceRecognition:
    ...
    def encode_faces(self):
        for image in os.listdir('D:\\project\\face_recognition\\webcam_face_recognition-master\\faces'):
        ...
        while True:
            ...
            if self.process_current_frame:
                ...
                for face_encoding in self.face_encodings:
                    ...
                    if self.method == 'lbph':
                        prediction, confidence = self.face_recognizer.predict(face_encoding)
                        if confidence > 80:
                            name = self.known_face_names[prediction]
                            confidence = str(round(100 - confidence, 2)) + '%'
                        else:
                            name = "Unknown"
                            confidence = '???'
                    else:
